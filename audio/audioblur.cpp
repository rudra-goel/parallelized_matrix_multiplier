...
int large_gauss_test(int argc, char **argv) {
    ...
    //this produces the main peak or the mean of the distribution appx halfway itno the array
    ...
    //takes the original audio sound and produces a new one
    // whose volume level is just as loud or less loud --> does not scale amplitude upward
    {
        float total = 0.0;
    ...
    //debug to see the gaussian distr.
#if 0
    for (int i = 0; i < GAUSSIAN_SIZE; i++)
    ...
    for (int ch = 0; ch < n_channels; ch++) {
        ...
        // CPU Blurring
        cout << "CPU blurring channel " << ch << "..." << endl;
        ...
        // Do the convolution
        //there is no need to "flip" explicitely the gaussian signal since it is symmetric
        // the flip phenomenon is happening when we multiply blur[j] by input[i-j]
        {
            for (int i = 0; i < GAUSSIAN_SIZE; i++) {
                for (int j = 0; j <= i; j++)
        ...
        // GPU blurring
        cout << "GPU blurring channel " << ch << "..." << endl;
        ...
        cout << "Comparing channel " << ch << "..." << endl;
        ...
        for (int i = 0; i < n_frames; i++) {
            if (fabs(output_data_host[i] - output_data[i]) < 1e-6) {
                ...
                error_count ++;
                if (error_count > 5) {
                    cerr << "Too many errors, aborting comparison" << endl;
                    break;
                }
            }
        }
        ...
        if (success)
            cout << endl << "Comparison successful on channel " << ch << endl;
